...
// loop unrolling
void vecAddUnrolled(int n, double *A, double *B, double *C)
{
  int i;
  for(i = 0; i<n; i = i+4)
  {
...
void vecMul(int n, double *A, double *B, double *C)
{
  int i;
  for(i = 0; i<n; i++)
  {
...
void vecMulWithIntrinsics(int n, double *A, double *B,
 double *C)
{
    int i;
    for(i = 0; i < n; i+=4) {
        __m256d aVec = _mm256_load_pd( &A[i] );
        __m256d bVec = _mm256_load_pd( &B[i] );
        __m256d cVec = _mm256_mul_pd( aVec, bVec );
        _mm256_store_pd( &C[i], cVec );
    }
}
...
void vecAddConst(int n, double *A, double *C)
{
    int i;
    for(i = 0; i<n; i+=4)
    {
      __m256d a = _mm256_load_pd( &A[i] );
      __m256d b = _mm256_set_pd( 100, 100, 100, 100 );
...
int main()
{
   ...
   //vecMulWithIntrinsics(NUM_ELEMENTS, data_A, data_B, data_C);
   vecAddConst(NUM_ELEMENTS, data_A, data_C);
